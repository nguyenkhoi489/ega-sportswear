...
if ( ! class_exists( 'YITH_WCAN_Admin_Premium' ) ) {
	...
	class YITH_WCAN_Admin_Premium extends YITH_WCAN_Admin_Extended {
		...
		public function __construct() {
			...
			// keep consistency between values of variations filtering and attribute lookup options.
			add_action( 'update_option_yith_woocommerce_variations_filtering', fn ( $old_value, $value ) => 'yes' === $value && update_option( 'woocommerce_attribute_lookup_enabled', 'yes' ), 10, 2 );
			add_action( 'update_option_woocommerce_attribute_lookup_enabled', fn ( $old_value, $value ) => 'no' === $value && update_option( 'yith_woocommerce_variations_filtering', 'no' ), 10, 2 );
		...
		public function add_general_options( $settings ) {
			...
			$additional_options_batch_2 = array(
				'variations_filtering' => array(
					'name'      => _x( 'Variations filtering', '[ADMIN] General settings page', 'yith-woocommerce-ajax-navigation' ),
					'desc'      => _x( 'Enable to make sure that only variable products with variations matching all the selected terms are shown in the set of results', '[ADMIN] General settings page', 'yith-woocommerce-ajax-navigation' ),
					'id'        => 'yith_woocommerce_variations_filtering',
					'type'      => 'yith-field',
					'default'   => 'no',
					'yith-type' => 'onoff',
				),
			...
			$options = yith_wcan_merge_in_array( $options, $additional_options_batch_2, 'ajax_filter' );
			...
			$options = yith_wcan_merge_in_array( $options, $additional_options_batch_3, 'show_clear_filter' );
			...
			$options = yith_wcan_merge_in_array( $options, $additional_options_batch_4, 'instant_filter' );
			...
			$options = yith_wcan_merge_in_array( $options, $additional_options_batch_5, 'general_section_end' );
		...
		public function add_preset_options( $settings ) {
			$settings['preset_layout'] = array(
				...
				'type'    => 'radio',
				'options' => YITH_WCAN_Presets_Factory::get_supported_layouts(),
		...
		public function add_filter_options( $settings ) {
			...
			$additional_options_batch_2 = array(
				...
				'order_options'                => array(
					...
					'options'  => YITH_WCAN_Filters_Factory::get_supported_orders(),
