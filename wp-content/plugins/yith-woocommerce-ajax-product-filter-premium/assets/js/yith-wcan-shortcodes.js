  ...
  yith_wcan_dropdown_createClass(YITH_WCAN_Dropdown, [{
    ...
    value: function getMatchingElements(search) {
      ...
      promise = new Promise(function (resolve) {
        ...
        }) : matchingElements; // then retrieve additional items
    ...
    value: function _formatItems(items) {
      ...
      var indexes = []; // remove duplicates and sort array of results
      ...
      return items.filter(function (_ref2) {
        ...
        if (-1 === indexes.indexOf(value)) {
          indexes.push(value); // checks if select has a related option.
  ...
  yith_wcan_preset_createClass(YITH_WCAN_Preset, [{
    ...
    value: function _initFilter($filter) {
      ...
      return this._maybeLoadFilter($filter).then(function ($filter) {
        ...
        _this4._initFilterChildren($filter); // init clear anchors
    ...
    value: function _initFilterChildren($filter) {
      ...
      this._initCollapsable($filter); // init show more link


      this._initShowMore($filter); // init custom inputs
    ...
    value: function _loadFilter($filter, filterHTML) {
      ...
      $filter.replaceWith($newFilter); // old filters are outdated, clear them until next .getFilters()
    ...
    value: function _loadItems($filter) {
      ...
      return new Promise(function (resolve) {
        $.ajax({
          ...
          var $items = $filter.find('.filter-content').children('.filter-items'); // append new items to filter existing ones.

          $items.append(data.success ? data === null || data === void 0 ? void 0 : (_data$data3 = data.data) === null || _data$data3 === void 0 ? void 0 : _data$data3.html : ''); // perform additional initalization of the new items in the filter.

          _this11._initFilterChildren($filter); // resolve promise returning jQuery set of the new elements.
    ...
    value: function maybeFilter($initiator) {
      ...
      this.maybeRegisterStatusChange(); // skip if filters are inhibited.
...
var YITH_WCAN_Preset_Premium = /*#__PURE__*/function (_YITH_WCAN_Preset) {
  ...
  yith_wcan_preset_premium_createClass(YITH_WCAN_Preset_Premium, [{
    ...
    value: function _initFilter($filter) {
      ...
      return yith_wcan_preset_premium_get(yith_wcan_preset_premium_getPrototypeOf(YITH_WCAN_Preset_Premium.prototype), "_initFilter", this).call(this, $filter).then(function () {
        _this4.maybeUpdateFiltersCount($filter);

        _this4.isHorizontal() && !yith_wcan_shortcodes.instant_horizontal_filter && $filter.on('click', function (ev) {
          return ev.stopPropagation();
        });
      });
    } // init tooltip
    ...
    value: function _initToggle($toggle, $container, $target) {
      ...
      $toggle.off('click').on('click', function (ev) {
        ...
        if (isHorizontal && !_this5.isMobile && $toggle.hasClass('filter-title')) {
          ...
          _this5.maybeCloseAllHorizontalToggles($target); // now open current one.
